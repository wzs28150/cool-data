...
const mergeOption = async () => {
  ...
  mergedOption.value.dataset = [mergedOption.value.dataset];
  ...
  for (let i = 0; i < dataLen; i++) {
    mergedOption.value.dataset[i + 1] = {
      transform: {
        type: 'filter',
        config: {
          dimension: mergedOption.value.dataset[0].dimensions[i + 1],
          '>': 0
        }
      }
    };
    ...
    seriesItemOption.itemStyle.borderWidth = 2;
    seriesItemOption.itemStyle.borderColor = new graphic.LinearGradient(
      0,
      0,
      0,
      1,
      [
        { offset: 0, color: toRgb('#ffffff', 1) },
        { offset: 1, color: toRgb('#ffffff', 0) }
      ]
    );
