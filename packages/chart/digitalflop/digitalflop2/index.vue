<template>
  <div class="digital-flop-2">
    <div class="digital-flop-2-inner">
      <div class="number-item ds-digital" v-for="(item, index) in numLength"
        :style="`font-size: ${mergedConfig.fontSize}px;width: ${mergedConfig.fontSize * 0.5}px;height: ${mergedConfig.fontSize}px; line-height: ${mergedConfig.fontSize}px; font-family: '${mergedConfig.fontFamily}';`">
        <span><i v-if="isNaN(numArray[index])">{{numArray[index]}}</i><i v-else :style="`transform :translate(-50%, -${numArray[index] * 10}%)`">0123456789</i></span>
      </div>
    </div>
  </div>
</template>
