...
export default {
  ...
  computed: {
    integration() {
      return {
        title: this.option.title ? this.option.title : '',
        // 主体离边框距离
        distance: this.option.distance ? this.option.distance : [0, 0],
        // 主体偏移值 (x,y)
        offset: this.option.offset ? this.option.offset : [0, 0],
        // 排序(max , min)优先
        sort: this.option.sort ? this.option.sort : '',
        // 鼠标点击事件
        lMouseClick: this.option.lMouseClick ? this.option.lMouseClick : false,
        // 鼠标移动事件
        lMouseMove: this.option.lMouseMove ? this.option.lMouseMove : false,
        // 颜色
        color: this.option.color ? this.option.color : ['#80FFA5', '#00DDFF', '#37A2FF', '#FF0087', '#FFBF00'],
        // 格式化字体输出
        ...
        tooltip: {
          ...
          fontColor: this.option.tooltip
            ? this.option.tooltip.fontColor
              ? this.option.tooltip.fontColor
              : '#000'
            : '#000', //  字体内部颜色
          fontSize: this.option.tooltip ? (this.option.tooltip.fontSize ? this.option.tooltip.fontSize : 14) : 14, // 字体大小
          backgroundColor: this.option.tooltip
            ? this.option.tooltip.backgroundColor
              ? this.option.tooltip.backgroundColor
              : '#fff'
            : '#fff', // tooltip背景
          formatter: this.option.tooltip
            ? this.option.tooltip.formatter
              ? this.option.tooltip.formatter
              : null
            : null, // 返回方法
          z: this.option.tooltip ? (this.option.tooltip.z ? this.option.tooltip.z : 999999) : 999999 // tooltip z-index层级
        },
        // 样式
        infoStyle: {
          stroke: this.option.infoStyle
            ? this.option.infoStyle.stroke
              ? this.option.infoStyle.stroke
              : false
            : false, //是否描边
          strokeColor: this.option.infoStyle
            ? this.option.infoStyle.strokeColor
              ? this.option.infoStyle.strokeColor
              : '#fff'
            : '#fff', // 描边颜色
          size: this.option.infoStyle ? (this.option.infoStyle.size ? this.option.infoStyle.size : null) : null, // 字体大小
          color: this.option.infoStyle ? (this.option.infoStyle.color ? this.option.infoStyle.color : null) : null, //颜色
          width: this.option.infoStyle
            ? this.option.infoStyle.width || this.option.infoStyle.width !== 0
              ? this.option.infoStyle.width
              : -10
            : -10, // 设置多少 就会在基础上加上设置的值
          offset: this.option.infoStyle
            ? this.option.infoStyle.offset
              ? this.option.infoStyle.offset
              : [0, 0]
            : [0, 0], // 字体x,y的偏移度
          setLineDash: this.option.infoStyle
            ? this.option.infoStyle.setLineDash
              ? this.option.infoStyle.setLineDash
              : [0, 0]
            : [0, 0], //虚线值
          highlightedColor: this.option.infoStyle
            ? this.option.infoStyle.highlightedColor
              ? this.option.infoStyle.highlightedColor
              : '#fff'
            : '#fff', //高亮颜色
          dotSize: this.option.infoStyle
            ? this.option.infoStyle.dotSize || this.option.infoStyle.dotSize !== 0
              ? this.option.infoStyle.dotSize
              : 4
            : 4 //点大小
        }
      }
  ...
  methods: {
    init() {
      if (!this.option.data || !this.option.data.length) return false
      this.removeEvent()
