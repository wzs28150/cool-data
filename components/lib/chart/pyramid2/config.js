function loadingFunnel(
  text,
  top,
  left,
  color,
  fontStyle,
  fontFamily,
  fontSize
) {
  return {
    text: text,
    top: top,
    left: left,
    textStyle: {
      color: color,
      fontStyle: fontStyle,
      fontFamily: fontFamily,
      fontSize: fontSize,
    },
  };
}
const arrData = [20, 40, 60, 80, 100];
const paperDataURI =
  'data:image/png;base64,';

const defaultOption = {
  title: [
    loadingFunnel(
      "指标二",
      "25%",
      "21%",
      "#d1ecff",
      "normal",
      "sans-serif",
      "14"
    ),
    loadingFunnel("40", "32%", "21%", "#EFC302", "normal", "Digital", "20"),
    loadingFunnel(
      "指标一",
      "18%",
      "52%",
      "#d1ecff",
      "normal",
      "sans-serif",
      "14"
    ),
    loadingFunnel("20", "24%", "55%", "#A5E104", "normal", "Digital", "20"),
    loadingFunnel(
      "指标四",
      "43%",
      "16%",
      "#d1ecff",
      "normal",
      "sans-serif",
      "14"
    ),
    loadingFunnel("80", "50%", "16%", "#0FE6BE", "normal", "Digital", "20"),
    loadingFunnel(
      "指标三",
      "33%",
      "65%",
      "#d1ecff",
      "normal",
      "sans-serif",
      "14"
    ),
    loadingFunnel("60", "39%", "68%", "#FF5A00", "normal", "Digital", "20"),
    loadingFunnel(
      "指标五",
      "56%",
      "75%",
      "#d1ecff",
      "normal",
      "sans-serif",
      "14"
    ),
    loadingFunnel("100", "62%", "78%", "#0078FF", "normal", "Digital", "20"),
  ],
  color: ["#bb0004", "#FFD48A"],
  tooltip: {
    show: true,
    formatter: function (params) {
      switch (params.name) {
        case "指标一":
          return "指标一 :&emsp;" + arrData[0];
        case "指标二":
          return "指标二 :&emsp;" + arrData[1];
        case "指标三":
          return "指标三 :&emsp;" + arrData[2];
        case "指标四":
          return "指标四 :&emsp;" + arrData[3];
        default:
          return "指标五 :&emsp;" + arrData[4];
      }
    },
  },
  grid: {
    // containLabel: true,
    left: "10%",
    top: "19%",
    bottom: "10%",
  },
  xAxis: {
    show: false,
    data: ["一季度", "二季度", "三季度", "四季度"],
    axisTick: {
      show: false,
    },
    axisLabel: {
      color: "#5EA2ED",
      interval: 0,
    },
    axisLine: {
      lineStyle: {
        color: "#1B5BBA",
      },
    },
  },
  yAxis: {
    show: false,
    splitLine: { show: false },
    axisLine: {
      lineStyle: {
        color: "#1B5BBA",
      },
    },
    axisLabel: {
      color: "#5EA2ED",
      interval: 0,
    },
  },
  series: [
    {
      type: "pictorialBar",
      // name: '案件归类',
      // z: 10,
      // label: {
      //     normal: {
      //         show: true,
      //         position: "top",
      //         formatter: "{c}",
      //         color: "#fff",
      //         fontSize: 13
      //     }
      // },
      /*data: [{
        name: '指标一',
        z: 100,
        value: 20,
        symbolSize: ['100%', '33%'],
        symbolPosition: 'center',
        symbolOffset: [142, '-1200%'],
        symbol: 'image://../img/Funnel1.png',
    }, {
        name: '指标二',
        z: 90,
        value: 40,
        symbolSize: ['75%', '33%'],
        symbolPosition: 'center',
        symbolOffset: [58.5, '-430%'],
        symbol: 'image://../img/Funnel2.png',
    }, {
        name: '指标三',
        z: 80,
        value: 60,
        symbolSize: ['125%', '33%'],
        symbolPosition: 'center',
        symbolOffset: [-28, '-165%'],
        symbol: 'image://../img/Funnel3.png',
    }, {
        name: '指标四',
        z: 70,
        value: 80,
        symbolSize: ['175%', '33%'],
        symbolPosition: 'center',
        symbolOffset: [-115, '-25%'],
        symbol: 'image://../img/Funnel4.png',
    },{
        name: '指标五',
        z: 60,
        value: 100,
        symbolSize: ['235%', '33%'],
        symbolPosition: 'center',
        symbolOffset: [-202, '65%'],
        symbol: 'image://../img/Funnel5.png',
    }]*/
      data: [
        {
          name: "指标一",
          z: 100,
          value: 20,
          symbolSize: ["100%", "63%"],
          symbolPosition: "center",
          symbolOffset: [152, "-600%"],
          symbol: 'image://' + paperDataURI,
        },
        {
          name: "指标二",
          z: 90,
          value: 40,
          symbolSize: ["200%", "33%"],
          symbolPosition: "center",
          symbolOffset: [-9.5, "-400%"],
          symbol: 'image://' + paperDataURI,
        },
        {
          name: "指标三",
          z: 80,
          value: 60,
          symbolSize: ["225%", "33%"],
          symbolPosition: "center",
          symbolOffset: [-6, "-145%"],
          symbol: 'image://' + paperDataURI,
        },
        {
          name: "指标四",
          z: 70,
          value: 80,
          symbolSize: ["275%", "33%"],
          symbolPosition: "center",
          symbolOffset: [-179, "-10%"],
          symbol: "image://../img/Funnel4.png",
        },
        {
          name: "指标五",
          z: 60,
          value: 100,
          symbolSize: ["335%", "33%"],
          symbolPosition: "center",
          symbolOffset: [-175, "75%"],
          symbol: "image://../img/Funnel5.png",
        },
      ],
    },
  ],
};

export default defaultOption;
